...
5. Record `yield` for every batch recipe — without it the calculator overstates materials by the batch size.
6. Prove calculations with `calculateRecipe` / `calculateBatch` smoke tests.
...
| `yield` | Units produced per craft — set it whenever a craft makes more than one (arrows, ammo, some food); omit for one-per-craft |
