...
interface ItemTypeaheadProps {
  ...
  /** Called after Enter resolves a craftable name (so parent can submit Calculate). */
  onEnterCommit?: (name: string) => void;
  placeholder?: string;
  className?: string;
}

export interface ItemTypeaheadHandle {
  /** Resolve the current draft query to a valid option, or null. */
  resolve: () => string | null;
}

export const ItemTypeahead = forwardRef<ItemTypeaheadHandle, ItemTypeaheadProps>(
  function ItemTypeahead(
    ...
    useImperativeHandle(
      ...
      () => ({
        resolve: () =>
          results[highlight] ??
          findClosestItem(query, options) ??
          (options.includes(value) ? value : null),
      }),
      [query, highlight, results, options, value],
    );
    ...
    function commitFromQuery(): string | null {
      const next =
        results[highlight] ??
        findClosestItem(query, options) ??
        (options.includes(value) ? value : null);
        ...
        <div className="relative">
          <Input
            ...
            aria-activedescendant={
              open && results[highlight] ? `${listId}-option-${highlight}` : undefined
            }
            value={query}
            ...
            onKeyDown={(e) => {
              if (e.key === "ArrowDown") {
                ...
                if (results.length > 0) {
                  setHighlight((i) =>
                    open ? (i - 1 + results.length) % results.length : results.length - 1,
                  );
                }
              } else if (e.key === "Enter") {
                e.preventDefault();
                const next = commitFromQuery();
                if (next) onEnterCommit?.(next);
